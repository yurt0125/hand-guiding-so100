...
class FurthestPointSampling(Function):
    ...
    def forward(ctx, xyz, npoint):
        ...
        """
        xyz = xyz.contiguous()
        if os.environ.get("GRASPNET_DEBUG_CONTIGUOUS", "0") == "1":
            print(
                "[pointnet2] fps input shape={} dtype={} device={} contiguous={} ext={}".format(
                    tuple(xyz.shape),
                    xyz.dtype,
                    xyz.device,
                    xyz.is_contiguous(),
                    getattr(_ext, "__file__", "unknown"),
                )
            )
...
class GatherOperation(Function):
    ...
    def forward(ctx, features, idx):
        ...
        return _ext.gather_points(features.contiguous(), idx.contiguous())
...
class ThreeNN(Function):
    ...
    def forward(ctx, unknown, known):
        ...
        """
        dist2, idx = _ext.three_nn(unknown.contiguous(), known.contiguous())
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features, idx):
        ...
        return _ext.group_points(features.contiguous(), idx.contiguous())
...
class BallQuery(Function):
    ...
    def forward(ctx, radius, nsample, xyz, new_xyz):
        ...
        """
        return _ext.ball_query(new_xyz.contiguous(), xyz.contiguous(), radius, nsample)
