...
def parse_args():
    ...
    p.add_argument("--pos-scale", type=float, nargs=3, default=[-0.20, -0.20, 0.10])
    ...
    p.add_argument("--frame-col", default="frame_index", help="CSV column that maps each row back to image frame")
    p.add_argument("--pitch-scale-deg", type=float, default=1.0, help="Scale applied to table-relative pitch before mapping to EE pitch")
    p.add_argument("--pitch-offset-deg", type=float, default=0.0, help="Constant added to mapped table-relative pitch")
...
def main():
    ...
    if args.pitch_col not in df.columns:
        raise ValueError(
            f"Missing required pitch column: {args.pitch_col}\n"
            "Tip: first export table-relative pitch into the CSV, then replay with this script."
        )

    if args.frame_col not in df.columns:
        # fall back to row index if not present
    ...
    def frame_to_target(i: int):
        nonlocal smooth_ee_xyz, smooth_g, smooth_pitch_deg
        ...
        mapped_pitch_deg = float(pitch_table_deg[i]) * args.pitch_scale_deg + args.pitch_offset_deg
        ...
        return target_gpos, gripper_q, raw_xyz, mapped_pitch_deg, smooth_pitch_deg
    ...
    with mujoco.viewer.launch_passive(mjmodel, mjdata) as viewer:
        while viewer.is_running():
            ...
            target_gpos, gripper_q, raw_xyz, mapped_pitch_deg, smooth_pitch_deg_now = frame_to_target(i)
            ...
            if args.verbose and i % 10 == 0:
                print(
                    ...
                    f"pitch_table_deg={pitch_table_deg[i]:.2f} "
