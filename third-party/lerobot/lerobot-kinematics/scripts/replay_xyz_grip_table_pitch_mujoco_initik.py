...
def main():
    ...
    # New startup initialization logic:
    # solve IK once for ee-home + ee-rpy-fixed, then start replay from that posture.
    ...
    target_qpos = np.concatenate(([init_qpos[0]], qpos_init_ik[:4], [init_qpos[5]]))
    ...
    if args.verbose:
        print(
            f"[INIT] start target_gpos=({init_target_gpos[0]:.3f}, {init_target_gpos[1]:.3f}, {init_target_gpos[2]:.3f}, "
            f"{init_target_gpos[3]:.3f}, {init_target_gpos[4]:.3f}, {init_target_gpos[5]:.3f}) init_ik={init_ik_success}"
        )
    ...
    def frame_to_target(i: int):
        ...
        return target_gpos, gripper_q, raw_xyz, raw_pitch_deg, filtered_pitch_deg, mapped_pitch_deg, smooth_pitch_deg, was_filtered
    ...
    with mujoco.viewer.launch_passive(mjmodel, mjdata) as viewer:
        while viewer.is_running():
            ...
            target_gpos, gripper_q, raw_xyz, raw_pitch_deg, filtered_pitch_deg, mapped_pitch_deg, smooth_pitch_deg_now, was_filtered = frame_to_target(i)
