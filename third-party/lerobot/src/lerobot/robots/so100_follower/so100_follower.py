...
class SO100Follower(Robot):
    ...
    def configure(self) -> None:
        config_retry = 3
        torque_retry = 5
        self.bus.disable_torque(num_retry=torque_retry)
        try:
            ...
            for motor in self.bus.motors:
                ...
                if motor == "gripper":
                    self.bus.write("Max_Torque_Limit", motor, 500, num_retry=config_retry)  # 50% of max torque to avoid burnout
            ...
            self.bus.enable_torque(num_retry=torque_retry)
